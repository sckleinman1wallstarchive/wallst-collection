 ...
 export function BudgetDialog({ onBack }: BudgetDialogProps) {
   ...
   const operatingPercent = metrics.operatingBudget > 0
     ? (metrics.operatingSpentThisMonth / metrics.operatingBudget) * 100
     : 0;
   ...
   return (
     <div className="space-y-6">
       ...
       {/* Alert Banners */}
       {(metrics.isOverDeployed || metrics.isInTransitHigh || metrics.isOperatingLow) && (
         <div className="space-y-2">
           ...
           {metrics.isOperatingLow && (
             <div className="flex items-center gap-3 p-4 bg-chart-4/10 border border-chart-4/20 rounded-lg">
               <Receipt className="h-5 w-5 text-chart-4" />
               <div>
                 <p className="font-medium text-chart-4">Operating Budget Running Low</p>
                 <p className="text-sm text-muted-foreground">
                   Less than 20% of this month's operating budget remains.
                 </p>
               </div>
             </div>
           )}
         ...
         <Card>
           <CardHeader className="pb-2">
             ...
             <p className="text-xs text-muted-foreground">
               Expenses, supplies, subscriptions, dividends
             </p>
           </CardHeader>
           <CardContent className="space-y-4">
             <div className="space-y-2">
               <div className="flex justify-between text-sm">
                 ...
                 <span className={metrics.isOperatingLow ? 'text-chart-4 font-semibold' : ''}>
                   {formatCurrency(metrics.operatingSpentThisMonth)} / {formatCurrency(metrics.operatingBudget)}
                 </span>
               </div>
               <Progress 
                 value={Math.min(operatingPercent, 100)} 
                 className={metrics.isOperatingLow ? '[&>div]:bg-chart-4' : ''}
               />
             </div>
             ...
             <div className="flex items-center justify-between text-sm text-muted-foreground">
               ...
               <span className="font-mono">
                 {formatCurrency(metrics.operatingRemaining)}
               </span>
             </div>
             
             <p className="text-xs text-muted-foreground border-t pt-3">
               12% of operating capital reserved for monthly costs
             </p>
           </CardContent>
         </Card>
         ...
         <Card>
           <CardHeader className="pb-2">
             <CardTitle className="text-base font-medium flex items-center gap-2">
               ...
               Deployment Budget (Weekly)
             </CardTitle>
             <p className="text-xs text-muted-foreground">
               Inventory purchases & acquisitions
             </p>
           </CardHeader>
           <CardContent className="space-y-4">
             <div className="space-y-2">
             ...
             <div className="flex items-center justify-between text-sm text-muted-foreground">
               <span>Remaining</span>
               <span className="font-mono">
                 {formatCurrency(metrics.deploymentRemaining)}
               </span>
             </div>
             
             <p className="text-xs text-muted-foreground border-t pt-3">
               45% weekly cap to prevent over-deployment
             </p>
           </CardContent>
         </Card>
       </div>
